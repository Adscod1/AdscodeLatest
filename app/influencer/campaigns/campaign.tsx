...
const CampaignsPage = () => {
  ...
  // New state for real campaign data
  ...
  const handleApplyToCampaign = async (campaignId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Failed to apply to campaign');
      }
              ...
              <div key={campaign.id} className="bg-white rounded-lg border border-gray-200 p-6">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      {selectedTab === 'applied' && campaign.applicationStatus && (
                        <span className={`px-2 py-1 text-xs font-medium rounded ${
                          campaign.applicationStatus === 'SELECTED' 
                            ? 'bg-green-100 text-green-700'
                            : campaign.applicationStatus === 'APPLIED'
                            ? 'bg-yellow-100 text-yellow-700'
                            : 'bg-gray-100 text-gray-700'
                        }`}>
                          {campaign.applicationStatus === 'SELECTED' ? 'Selected' : 
                           campaign.applicationStatus === 'APPLIED' ? 'Application Pending' : 
                ...
                <div className="flex items-center gap-6 mb-4 text-sm text-gray-600 flex-wrap">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span>{campaign._count.applicants} {campaign._count.applicants === 1 ? 'Applicant' : 'Applicants'}</span>
                  </div>
                  {campaign.influencerLocation && (
                    <div className="flex items-center gap-1">
                      <MapPin className="w-4 h-4" />
                      <span>{campaign.influencerLocation.country}</span>
                    </div>
                  )}
                </div>
                ...
                {campaign.targets && campaign.targets.length > 0 && (
                  <div className="mb-4">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {campaign.targets.slice(0, 3).map((target: string, index: number) => (
                ...
                <div className="flex gap-3">
                  ...
                  {selectedTab === 'applied' && campaign.applicationStatus === 'APPLIED' && (
                    <button 
                      className="flex-1 px-4 py-2 bg-gray-100 text-gray-500 text-sm font-medium rounded-lg cursor-not-allowed"
                      disabled
                    >
                      Applied
                    </button>
